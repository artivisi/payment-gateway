...
- [~] **Per-bank CSV ingestion (the real follow-up).** `SettlementCsvParser` currently handles **one** canonical layout (`vaNumber,bankReference,amount,transactionTime`, ISO-8601). Indonesian banks generally **do not** offer a settlement pull API — ops download a CSV from the bank's cash-management portal (or a PDF statement). So the work is a **per-bank/per-escrow column+format mapping** (delimiter, headers, `dd/MM/yyyy`, thousands-separator amounts, debit/credit columns, VA embedded in a remark field), letting ops upload the bank's raw export. PDF: avoid in-app parsing — prefer the CSV export; per-bank PDF extractor only if a bank is PDF-only.
- [ ] ~~`pullSettlement` outbound REST API~~ — **dropped for launch banks** (not offered). Revisit only per-bank if one provides host-to-host/API; optional **SFTP auto-ingest** if a bank drops daily files. (This is where the BANK_HOSTED/outbound `BankProvider` interface would extract, if ever needed.)
...
- [x] Brand + shell: Tailwind v4 `@theme` (ArtiVisi primary #2e3192 / accent #58c034), standalone-CLI build → `static/css/app.css` (binary gitignored; **built by Maven via the `tailwind` profile** when `.tools/tailwindcss` is present); logos + branded nav; dashboard with counts. `/` → `/admin`.
...
- [x] DevSecOps gates: **SpotBugs** ✅ (local `verify` gate, 0 findings) · **CI** (`ci.yml`) · **CodeQL** (`codeql.yml`) · **OWASP Dependency-Check** (`dependency-check.yml`, needs `NVD_API_KEY`) · **SonarCloud** (`sonarcloud.yml` + pom `sonar.*`, needs `SONAR_TOKEN` + project) · **OWASP ZAP** baseline (`zap.yml`, boots app vs throwaway Postgres). **All GitHub workflows are `workflow_dispatch`-only** (manual, to conserve Actions quota) — the suite is run locally with `mvn verify`; SpotBugs is the enforced local gate. **JaCoCo** (0.8.13, works on Java 25) runs under `verify` → `target/site/jacoco/jacoco.xml` feeds SonarCloud coverage. **Tailwind** CSS is built by Maven via a `tailwind` profile auto-activated when `.tools/tailwindcss` exists (CI/fresh-clone → committed `app.css` used).
- [x] **Admin authentication + PCI-DSS control baseline** (out of PCI scope — no card data — but adopting controls; see `docs/security/pci-dss.md`). Spring Security on `/admin`: operator accounts (`operator`); **permission-based RBAC** — fixed `Permission` enum (feature→permission, `hasAuthority`) with **data-driven roles** (`role`/`role_permission`, one per operator, runtime-editable via Roles UI; built-in ADMIN/OPERATOR/AUDITOR + custom) (Req 7), bcrypt + min-12 password (Req 8.3), lockout 10/30m (`AuthEventListener`), 15-min idle session, **forced password change + TOTP MFA enrolment** on first login (`AdminAccessGuardFilter`, `TotpService`; Req 8.4), fail-loud bootstrap admin. Operator CRUD UI (ADMIN). Authenticated **actor on every `AuditEvent`** + auth-event audit (Req 10). **Per-provider bank IP allowlist** (`bank_ip_rule`, `BankIpAllowlistFilter`, admin-managed): source IP not allowlisted → 403; empty = unrestricted; replaces network-layer filtering for BSI/CIMB. Tests: `OperatorAuthIntegrationTest`, `BankIpAllowlistIntegrationTest`, Playwright login+MFA. **Phase B deferred:** password rotation, auto-trip escalation, session concurrency.
