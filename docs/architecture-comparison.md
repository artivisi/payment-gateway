...
Full methodology and both systems' numbers across two runs each are in [`payment-gateway-evtsrc/scenarios/perf_benchmark_report.md`](https://github.com/artivisi/payment-gateway-evtsrc/blob/main/scenarios/perf_benchmark_report.md). A short summary and this repo's own results are in [`docs/benchmark-report.md`](benchmark-report.md), including a real finding from the second run: performance degrading under sustained load on a small hot-row dataset, not benchmark noise.
...
| Section 3 — RDBMS throughput is sufficient, Kafka-scale is premature at this workload | Mixed: a first run absorbed the 2,000 TPS ramp using only 26 of 2,000 allotted virtual users with 0% errors, but a second run against the same never-reset database (115,000+ accumulated payment rows concentrated on 2 of 8 charges) needed 662 VUs and p99 jumped to 472ms — a real row-lock-contention characteristic under sustained load, not a capacity ceiling in the sense section 3 meant. Neither system was pushed to an actual hardware-limited ceiling, so the "5,000-10,000+ TPS" figure in section 3 above remains unverified. See `docs/benchmark-report.md` for detail. |
