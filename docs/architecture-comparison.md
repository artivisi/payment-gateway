...
| **Throughput Capacity** | Hundreds to thousands of ACID TPS per single node; measured p99 of 51ms sustaining a 2,000 TPS ramp with 156 concurrent virtual users. | Measured saturating well before the same 2,000 TPS ramp completed — p99 in the 1.2–3.2 second range, needing 1,200+ concurrent virtual users for the same throughput, tail latency still climbing at the end of the ramp rather than recovering. Not comparable at the tested scale; see Post-Selection Validation. |
...
## Post-Selection Validation (2026-07-29)
...
Full methodology and both systems' numbers are in [`payment-gateway-evtsrc/scenarios/perf_benchmark_report.md`](https://github.com/artivisi/payment-gateway-evtsrc/blob/main/scenarios/perf_benchmark_report.md) — one clean run for this repo, two independently-reset clean runs for the event-sourced comparison (repeated specifically to check whether an anomaly found in the first run was a fluke; it wasn't). A short summary and this repo's own results are in [`docs/benchmark-report.md`](benchmark-report.md).

### What was confirmed

| Trade-off argument (above) | Measured outcome |
|---|---|
| Section 1 — synchronous protocols favor fewer hops | Confirmed: this system's median latency (858 microseconds) was 14–33x lower than the event-sourced variant's (12.09ms and 27.9ms across its two runs) under the identical BSI protocol workload, and the gap widens sharply under load — see Section 3 below. |
| Section 2 — row locking is simpler/safer than distributed invariant enforcement | Confirmed, and by a sharper failure mode than anticipated, twice over. Earlier: the event-sourced variant had to re-implement the OPEN-vs-INSTALLMENT charge closing rule in two separate stores (its Kafka Streams topology and its Postgres projection), and the two silently disagreed until the mismatch was found. New: under the saturation load described in Section 3, its financial-correctness audit failed in *both* independently-reset runs — one payment recorded twice (once accepted, once flagged as a double settlement) for the same bank reference, each time on a charge that had just been created. This repo's single ACID transaction per payment has no code path that can produce that outcome; its audit passed with zero mismatches. See `perf_benchmark_report.md` §4. |
| Section 3 — RDBMS throughput is sufficient, Kafka-scale is premature at this workload | Confirmed, more starkly than expected: on the identical 2,000 TPS ramp on the same shared hardware, this repo needed only 156 of 2,000 allotted virtual users and held p99 at 51ms, while the event-sourced variant needed 1,200+ VUs and posted p99s of 3.22s and 1.16s across its two runs — worse, its tail latency was still climbing at the end of the ramp-down window rather than recovering, the signature of a backlog that hadn't drained by the time the test ended. Neither system was pushed to an actual hardware-limited ceiling, so the "5,000-10,000+ TPS" figure in section 3 above remains unverified for this repo specifically, but "comparable capacity" is no longer a defensible reading of the data. See `perf_benchmark_report.md` §3 for the per-stage breakdown and the (unconfirmed, plausible) mechanism. |
| Section 4 — lower operational footprint | Confirmed in effort, not just infrastructure: getting the event-sourced comparison implementation to correct behavior took substantially more fixes (a missing framework dependency that silently broke persistence, three iterations to get its own test harness right, the two-store invariant bug above) than this system needed at any point — and the Section 2 defect above surfaced only under a load test this repo passed cleanly. |
