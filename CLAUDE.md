...
- **OPEN** — each payment adds to `cumulativePaid` (**shared across siblings**, same repricing); the charge **never auto-completes** — there is no cap, no terminal `PAID` state, siblings stay `ACTIVE` indefinitely. This is for a standing/always-active account (e.g. a donation VA), where `amount` is a nominal display target, not an enforced ceiling. Capped, target-amount billing across multiple payments is the **consumer's** concern: the receivables application holds the schedule and reprices one CLOSED charge over time. There is deliberately no `INSTALLMENT` type — it existed until 2026-08, never carried a production charge, and a bank-side instalment VA is bank-specific.
